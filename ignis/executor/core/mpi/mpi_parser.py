...
class MpiParser:
	...
	def __init__(self, cache=True):
		...
		raw_macros = subprocess.check_output(['gcc', '-dM', '-E', '/usr/local/include/mpi.h']).decode().split("\n")
...
if __name__ == '__main__':
	...
	for macro in mpi.getMacros():
		...
		if go_type == "*C.void":
			...
			unsafe = True
		go_source.write("var " + macro.name + " " + go_type + " = ")
	...
	for f in mpi.getFunctions():
		...
		i = 0
		for arg in f.args:
			go_source.write(arg.name + " " + mpi.getVarTypeAsGo(arg.tp))
			i += 1
			if i < len(f.args):
				go_source.write(", ")
		go_source.write(") ")
		if f.rtype == "int":
			...
			go_source.write(mpi.getVarTypeAsGo(f.rtype))
		go_source.write(" {\n   return ")
		if f.rtype == "int":
